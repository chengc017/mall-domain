...
public interface IBaseDAO<T> {
	 ...
	 * 
	 * @param detachedCriteria
	 *            the domain query criteria, include condition and the orders.
	 * @return
	 * @author <a href="mailto:rory.cn@gmail.com">somebody</a>
	 */
	 ...
	 */
	public List<T> getListByPosition(final int begin, final int pageSize,
	 ...
	 */
	public List<T> getListByNamedParameter(final int begin, final int pageSize,
			final String hql, final Map map);

	/*
	 * 如：hql = " from User user where user.username=:name and user.id=:id"
	 * 数组names 为{"name","id"} 数组values 为 {"haoxw",new Long(1)}
	 */
	public List<T> getListByNamedParameter2(final int begin,
			final int pageSize, final String hql, final String[] names,
			final Object[] values);

	/*
	 * 返回上面的总记录数
	 */
	public int getTotalCountByNamedParameter2(String hql, String[] names,
			Object[] values);
